class CommonConstants {
  //Strings
  static const String bottomFooter= 'copyright at @alofra';

  static const String test = 'test';
  static const num testNum = 1;
  static const double headerText = 60.0;
  static const double largeText = 40.0;
  static const double meduimText = 20.0;

  static const double normalText = 22.0;
  static const double smallText = 16.0;

  //fonts
  static const String largeTextFont="Segoe_UI";
  static const String introTextFont="Roboto";


  //button size
  static double roundedHeight =50;
  static double textButton =20;
  static double horizontalPaddingButton =20;
  static double verticalPaddingButton =10;

  //text
  static double hintSizes =18;

  static double paddingBottom =40;
  static double paddingHorizontal =12;


}